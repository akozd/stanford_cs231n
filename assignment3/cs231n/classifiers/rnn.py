...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        #forward pass
        aff, cache1 = affine_forward(features,W_proj,b_proj) #shape (N,H)
        emb, cache2 = word_embedding_forward(captions_in, W_embed) #shape (N,T,W)
        if self.cell_type == 'rnn':
            h, cache3 = rnn_forward(emb, aff, Wx, Wh, b) #shape (N, T, H)
        elif self.cell_type == 'lstm':
            h, cache3 = lstm_forward(emb, aff, Wx, Wh, b)
        else:
            raise ValueError('Invalid cell_type "%s"' % cell_type)
        taff, cache4 = temporal_affine_forward(h, W_vocab, b_vocab)
        
        #loss
        loss, dldx = temporal_softmax_loss(taff, captions_out, mask, verbose=False)
        
        #backprop dat ass up
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dldx, cache4)
        if self.cell_type == 'rnn':
    ...
    def sample(self, features, max_length=30):
        ...
        # Unpack parameters
        W_proj, b_proj = self.params['W_proj'], self.params['b_proj'] #used
        W_embed = self.params['W_embed']
        Wx, Wh, b = self.params['Wx'], self.params['Wh'], self.params['b'] #used
        W_vocab, b_vocab = self.params['W_vocab'], self.params['b_vocab']
